...
class PageWordIndex:
    ...
    def __init__(self, sp, unit, page_height, grid_nx=None, grid_ny=None):
        ...
        ids = [None]*N; texts = [None]*N

        for i, c in enumerate(cells):
            r = c.rect
            # y min/max depends on origin
    ...
    def query_bbox(self, ql, qt, qr, qb, ios=0.8, scale=1.0):
        """Query with TOPLEFT-origin bbox. Returns token dicts like your current code."""
        if not self.ids:
            return []

        # Gather candidates from bins overlapped by query box
        cell_w = self.W / self.gx
        cell_h = self.H / self.gy
        ...
        # Deduplicate and vectorized IoS filter
