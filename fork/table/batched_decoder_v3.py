...
# Optimized Batched AR decoder with GPT-5 Phase 1 optimization implemented
        ...
        for step in range(Tmax):
            ...
            if append_idx.numel() > 0:
                if append_idx.numel() > 0:
                    ...
                    end_idx = (m_emit_bbox & (~first_lcel) & append_mask & (~finished)).nonzero(as_tuple=False).squeeze(
                        1)
                    if end_idx.numel() > 0:
